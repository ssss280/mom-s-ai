...
> ## 🚫 AI / 贡献者必读：改版本号前必须先问人
>
> **不要自己决定版本号。** 改 `version.py` 的 `__version__`、增删 CHANGELOG 版本标题、
> 执行 `release.py --apply/--release`、打版本标签——**都必须先向用户请求并等到明确同意**。
>
> - 完整规则见 **[AGENTS.md](AGENTS.md)**（AI 助手的入口文件，第一条就是这条）。
> - 技术上也有强制：`release.py` 缺 `--approved <版本号>` 会直接拒绝执行（退出码 4）。
> - 原因：版本号是对外承诺，改错会影响所有已安装用户的更新判断，推送到公开仓库就收不回来；
>   本项目**已经因为自动跳号踩过坑**（早期出现过两组同名版本）。
