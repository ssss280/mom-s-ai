...
├── web_search.py        # 免密钥联网搜索（Bing/360 主攻 + 搜狗/百度备用 + 相关性过滤）
...
- `search(query, count)`：返回 `{"query", "engine", "results": [{"title","url","snippet","engine"}], "error"}`，
  任何失败都写进 `error`，不抛异常
- `build_context(results)`：把结果拼成给模型看的编号上下文（`[1] 标题 / URL / 摘要`）
- **并行查 Bing / 360 / 搜狗 / 百度**，合并去重后按相关性排序；DuckDuckGo 只在前面都没结果时兜底
  - 早期版本是"谁先出结果用谁"，被真实案例打脸：搜「2026香港秋季照明展」时 Bing 返回一堆
    「2026 年日历/放假安排」，它"有结果"就把搜到了展会时间的搜狗结果挤掉了
  - 本机实测：DuckDuckGo 直连超时（国内不通）；搜狗、百度**很容易触发安全验证**
    （连续十几次查询就被拦，页面变成「百度安全验证」），360 明显更抗压
- **相关性过滤分两档**（打分前先用 `term_text()` 剔掉问句成分：什么时候/是什么/帮我查一下/呢/的…）：
  ...
  - `≥ MIN_RELEVANCE`（0.3）= 可信结果，正常喂给模型
  - `FALLBACK_MIN`（0.2）~ 0.3 = **低相关参考**：仍交给模型，但注入 `SEARCH_LOW_RELEVANCE_PROMPT`
    要求它声明"没检索到很匹配的资料、仅供参考"——模型判语义比 bigram 打分靠谱，也好过直接说"没查到"
  - 低于 0.2 直接丢弃（实测日历垃圾 0.16，正好挡在门外）
- **追问要结合上下文**（`is_follow_up()` / `build_query()`）：「2026年的呢」「那地点呢」这类句子自己没检索价值，
  必须拼上上一句（实测日志里出现过直接拿「2026年的呢」去搜）。只有年份/以"呢"结尾/以"那"开头才算追问，
  `北京天气` 这种短查询不会被误拼
- **引擎分两批打**：先用 `PRIMARY_ENGINES`（Bing + 360，最抗压），没结果才拉 `SECONDARY_ENGINES`
  （搜狗 + 百度，中文覆盖好但很容易触发安全验证）。一次并行打 4 个的话，正常用几次就会被集体拦
- **反爬识别与冷却**：没有结果 + 页面异常短 + 出现"安全验证/验证码"字样 → 判定被拦，
  该引擎冷却 `COOLDOWN`（5 分钟）不再请求（`EngineBlocked` + `_blocked_until`；10 分钟太长，等于把自己饿死）
