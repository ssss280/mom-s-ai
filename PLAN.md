...
├── web_search.py        # 免密钥联网搜索（Bing/360/搜狗/百度 并行 + 相关性过滤）
...
- **相关性过滤**（`relevance()`）：中文按 2 字滑窗（bigram）、英文数字按词，命中数/总项数，
  标题命中加权、整体命中加分；低于 `MIN_RELEVANCE`（0.3）的结果**直接丢掉**，
  宁可返回空也不把无关网页喂给模型
- **反爬识别与冷却**：没有结果 + 页面异常短 + 出现"安全验证/验证码"字样 → 判定被拦，
  该引擎冷却 10 分钟不再请求（`EngineBlocked` + `_blocked_until`）
